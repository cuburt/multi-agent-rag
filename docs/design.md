...
### CI/CD & Rollback Plan
- **CI ([`.github/workflows/ci.yml`](../.github/workflows/ci.yml)):** Four jobs. The first two gate every push/PR; the last two only fire on pushes to `main` (with `release` also firing on `vX.Y.Z` tag pushes).
  - `unit-tests` — spins up a `pgvector/pgvector:pg16` service container, installs `requirements.txt`, runs `pytest tests/` with **no LLM keys configured**. Caching, RBAC routing, redaction, retrieval shape, and metrics aggregation are exercised against stubs / a real DB.
  - `integration-redteam` — depends on `unit-tests`, brings up the API against pgvector, seeds the DB, and runs `evals/red_team.py`. Gated on `OPENROUTER_API_KEY` being set as a repo secret; on forks (or with no secret) it skips cleanly.
  - `release` — depends on `unit-tests`. Builds a linux/amd64 image with buildx (using a `:buildcache` registry layer so subsequent builds are incremental) and pushes to Docker Hub `cuburt4798/multi-agent-rag`. Tags are produced by `docker/metadata-action`: pushes to `main` get `:main`, `:sha-<short>`, `:latest`; pushes of `vX.Y.Z` tags additionally get `:X.Y.Z`, `:X.Y`, `:X`.
  - `deploy` — depends on `release`. Authenticates to GCP via the `GCP_SA_KEY` service account, writes a Cloud Run env-vars yaml from repo secrets (assembling `DATABASE_URL` from `POSTGRES_PASSWORD` + a `/cloudsql/<project>:us-central1:multi-agent-postgres` unix-socket path), and runs `gcloud run deploy multi-agent-rag` against the **content-addressable digest** produced by `release`, not a tag. This means `:latest` could be overwritten by a later build without ever changing what's serving production.

  The unit-test gate is **never** skipped. The red-team job becomes a hard gate as soon as a maintainer adds the secret.
- **CD topology:** Same single image runs in three modes — `docker compose` (compose injects `DATABASE_URL` and overrides CMD to seed-then-serve), local dev (`uvicorn --reload`, no container), and Cloud Run (the Dockerfile CMD honors `$PORT` and the deploy job attaches `--add-cloudsql-instances` so the auth-proxy socket appears at `/cloudsql/...`). The Streamlit frontend is not part of the Cloud Run deploy in this iteration; only the FastAPI API is served.
- **Caching ([`src/agents/graph.py`](../src/agents/graph.py)):** Per-tier in-memory TTL+LRU cache (default 256 entries) wrapped around `get_llm_response`. ROUTER 5 min (classifications are stable), SYNTHESIS 60 s (absorbs retries / re-asks), AGENTIC disabled (its prompts include the patient's live appointment list — caching would mask state drift). All TTLs are env-overridable (`LLM_CACHE_TTL_*_S`). Hits are logged as `llm_cache_hit`.
- **Model rollback (no code change required):** The three primary models are env-driven (`ROUTER_MODEL`, `AGENTIC_MODEL`, `SYNTHESIS_MODEL`). On Cloud Run, a rollback is `gcloud run services update multi-agent-rag --update-env-vars ROUTER_MODEL=<old>` — a new revision is created in seconds and traffic shifts atomically. The hardcoded fallback chain in `MODEL_CHAINS` provides automatic in-flight rollback per call: a 5xx or rate-limit on the primary advances to the next model in the same tier, no human in the loop.
- **Code rollback:** Every `main` commit produces a `:sha-<short>` tag *and* the deploy targets the digest, so rollback is either `gcloud run services update-traffic multi-agent-rag --to-revisions <previous>=100` (instant, no rebuild) or — for an older build that's been GC'd from revision history — redeploying a specific image tag (`gcloud run deploy ... --image cuburt4798/multi-agent-rag:<tag>`). For releases cut via `vX.Y.Z`, the semver tags survive on Docker Hub independent of Cloud Run revision retention. State lives in Cloud SQL (Postgres), migration-managed by `init_db()` (idempotent table creation + `CREATE EXTENSION IF NOT EXISTS vector`).
